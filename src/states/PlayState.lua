    ...
    for k, ball in pairs(self.balls) do 
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
                ...
                -- Randomly spawn a powerup when hitting a brick
                if math.random(100) < 20 and not brick.locked then  -- 20% chance of getting a powerup
                    if math.random(100) < 20 then -- 20% chance of getting a key powerup
                        powerupType = 10
                    else
                        powerupType = math.random(9)
                    end
                    table.insert(self.powerups, Powerup(powerupType, ball.x, ball.y))
                end
        ...
        for k, powerup in pairs(self.powerups) do
            ...
            if powerup:collides(self.paddle) then
                if powerup.powerupType < 10 then
                     self:bonusBalls()  
                end       
                if powerup.powerupType == 10 then
                    self.key = true
...
function PlayState:render()
    ...
    -- Show key powerup if player has the key powerup
    if self.key then
